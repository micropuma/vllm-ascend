...
# DBO + FlashComm1 精度验证：FC1=ON + DBO=OFF vs FC1=ON + DBO=ON
# 隔离 DBO 变量，验证 DBO 在 FlashComm1 开启时不改变模型输出
#
# 用法: bash test_dbo_fc1_precision.sh
...
LOG_FILE="${DEMOS_DIR}/test_fc1_precision_${TIMESTAMP}.log"
mkdir -p "$OUT_DIR"
...
# ── 发请求脚本 ──
PRECISION_PY="${OUT_DIR}/_fc1_precision_test.py"
...
# ── Main ──
log "DBO + FlashComm1 精度验证: ${NUM_PROMPTS} prompts × in=${INPUT_LEN} out=${OUTPUT_LEN}, seed=$SEED"
log "两组均 FC1=ON, HCCL=AI_CPU, FC2=OFF"
log "Log: $LOG_FILE"

run_config "fc1_nodbo" 0
run_config "fc1_dbo"   1

NODBO_FILE="${OUT_DIR}/fc1_nodbo_precision_${TIMESTAMP}.jsonl"
DBO_FILE="${OUT_DIR}/fc1_dbo_precision_${TIMESTAMP}.jsonl"
