...
if [[ "$ENABLE_PROFILER" == "1" ]]; then
  ...
  PROFILER_CONFIG="{\"profiler\":\"torch\",\"torch_profiler_dir\":\"${TORCH_PROFILER_DIR}\",\"torch_profiler_with_memory\":true,\"torch_profiler_record_shapes\":${PROFILER_RECORD_SHAPES},\"torch_profiler_with_stack\":${PROFILER_WITH_STACK},\"torch_profiler_use_gzip\":true,\"torch_profiler_with_flops\":false,\"max_iterations\":${PROFILER_MAX_ITERATIONS}}"
