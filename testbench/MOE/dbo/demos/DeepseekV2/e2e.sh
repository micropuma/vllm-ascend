...
# ║  Each case defines: LABEL, SERVER script, and environment overrides.         ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

# ── Case 1 ─────────────────────────────────────────────────────────────────────
CASE1_LABEL="${CASE1_LABEL:-baseline}"
CASE1_SERVER="${CASE1_SERVER:-deepseek-v2-server.sh}"
declare -a CASE1_ENV=(
  VLLM_ASCEND_ENABLE_FLASHCOMM1="${CASE1_FC1:-0}"
  VLLM_ASCEND_ENABLE_DBO="${CASE1_DBO:-0}"
...
CASE2_SERVER="${CASE2_SERVER:-deepseek-v2-dbo-server.sh}"
CASE2_COLD_CACHE="${CASE2_COLD_CACHE:-1}"   # 0 = reuse warm cache, 1 = fresh cold cache
declare -a CASE2_ENV=(
  VLLM_ASCEND_ENABLE_FLASHCOMM1="${CASE2_FC1:-1}"
  VLLM_ASCEND_ENABLE_DBO="${CASE2_DBO:-1}"
...
RUN_ROOT=${RUN_ROOT:-"./deepseek-v2-tp-dbo-${RUN_ID}"}
PORT=${PORT:-8001}
...
cleanup_device_processes() {
    ...
    # A crashed API server can leave detached vLLM engine/workers holding HBM.
    local stale_workers
    local stale_servers
...
cleanup() {
  ...
  # Always run fallbacks, including when the process group exited early.
  cleanup_device_processes
...
# ── Start server with console + file output ───────────────────────────────────
run_server() {
  local script=$1 log_file=$2
  shift 2
  ...
  env "$@" setsid bash -c "bash "$SCRIPT_DIR/$script" 2>&1 | tee "$log_file"" &
  server_pid=$!
  wait_server || {
    echo "════════════════════════════════════════════════════════════"
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║  PHASE 1: ${CASE1_LABEL}  (server=${CASE1_SERVER})"
echo "╚══════════════════════════════════════════════════════════════════════╝"

rm -f "$RESULT1"

run_server "$CASE1_SERVER" "$RUN_ROOT/${CASE1_LABEL}.log" \
  PORT="$PORT" "${CASE1_ENV[@]}"
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║  PHASE 2: ${CASE2_LABEL}  (server=${CASE2_SERVER})"
echo "╚══════════════════════════════════════════════════════════════════════╝"
...
run_server "$CASE2_SERVER" "$RUN_ROOT/${CASE2_LABEL}.log" \
  PORT="$PORT" "${CASE2_ENV[@]}" "${CASE2_CACHE_ENV[@]}"
