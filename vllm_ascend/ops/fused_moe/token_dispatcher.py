...
class TokenDispatcherWithAll2AllV(MoETokenDispatcher[MoEAllToAllCombineMetadata]):
    ...
    def token_dispatch(
        ...
        if forward_context.dbo_enabled:
            torch.ops.vllm.dbo_moe_prepare_hook(permutated_local_input_tokens, is_record=True)
        if with_quant:
        ...
        if forward_context.dbo_enabled:
            torch.ops.vllm.dbo_moe_prepare_hook(global_input_tokens, is_record=False)
