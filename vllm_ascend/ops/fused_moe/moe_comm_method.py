...
def get_moe_comm_method(moe_comm_type: MoECommType | None, ubatch_num=0) -> MoECommMethod | None:
    moe_comm_method = _MoECommMethods.get(moe_comm_type)
    if moe_comm_method is not None:
        # only support dbo for now
        if not 0 <= ubatch_num < len(moe_comm_method):
            raise ValueError(
                f"Invalid ubatch_num={ubatch_num}, "
                f"available instances={len(moe_comm_method)}, "
